...
Для удобства распараллеливания воткнул Supervisor в контейнер php-cli. По дефолту запускается 10 воркера-консюмера, 
их количество можно изменить в `docker/conf/backend/messenger.conf:9`, так же 3 симфони-процессов для получения
постов из страницы, которые в свою очередь кидают посты в мессенджер. Сообщения обрабатываются батчем по 8 штук с
...
2. [ ] 2 - Добавить логирование времени - сколько времени занял процесс парсинга
...
7. [ ] 7 - Переписать хранение прокси. Сделать хранение обычным txt-файлом, чтобы было удобно поменять их на другой машине
8. [ ] 8 - Улучшить алгоритм парсинга - на каждый message повесить обработку не 1 поста, а пачки постов (batch), и запросы
