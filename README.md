...
1. Запускаем `php bin/console post:get-list` - начнется процесс сбора всех постов. В среднем скрипт собирает все посты
примерно за ~1 час (учитывая RetryHttpClient и сампописный ретрай) в 100 процессах со 100 прокси - по 1 прокси на 
каждый процесс.
...
6. Создать отдельную фабрику для ```CreatePostInputDTO```
